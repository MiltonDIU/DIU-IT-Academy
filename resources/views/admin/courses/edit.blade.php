        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $course->slug) }}" required>
                    @if($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('introduction') ? 'is-invalid' : '' }}" name="introduction" id="introduction">{!! old('introduction', $course->introduction) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('about_this_course') ? 'is-invalid' : '' }}" name="about_this_course" id="about_this_course">{!! old('about_this_course', $course->about_this_course) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="text" name="duration" id="duration" value="{{ old('duration', $course->duration) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control datetime {{ $errors->has('class_start_date') ? 'is-invalid' : '' }}" type="text" name="class_start_date" id="class_start_date" value="{{ old('class_start_date', $course->class_start_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('class_end_date') ? 'is-invalid' : '' }}" type="text" name="class_end_date" id="class_end_date" value="{{ old('class_end_date', $course->class_end_date) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('course_content_types') ? 'is-invalid' : '' }}" name="course_content_types[]" id="course_content_types" multiple required>
                        @foreach($course_content_types as $id => $course_content_type)
                            <option value="{{ $id }}" {{ (in_array($id, old('course_content_types', [])) || $course->course_content_types->contains($id)) ? 'selected' : '' }}>{{ $course_content_type }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('skill_covereds') ? 'is-invalid' : '' }}" name="skill_covereds[]" id="skill_covereds" multiple>
                        @foreach($skill_covereds as $id => $skill_covered)
                            <option value="{{ $id }}" {{ (in_array($id, old('skill_covereds', [])) || $course->skill_covereds->contains($id)) ? 'selected' : '' }}>{{ $skill_covered }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('required_skills') ? 'is-invalid' : '' }}" name="required_skills[]" id="required_skills" multiple>
                        @foreach($required_skills as $id => $required_skill)
                            <option value="{{ $id }}" {{ (in_array($id, old('required_skills', [])) || $course->required_skills->contains($id)) ? 'selected' : '' }}>{{ $required_skill }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active" required>
                        ...
                        @foreach(App\Models\Course::IS_ACTIVE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $course->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
